...
## Entity Resolution for linking patient records at 100s of million records scale. 
...
![Hybrid entity resolution pipeline](patient_matching.svg)
...
In parallel, [embedding-based entity resolution](https://github.com/sinaghadermarzi/embedding-based-entity-resolution) is an open, reproducible investigation of the design choices for large-scale person matching. It tests learned embeddings, candidate generation, clustering, and deterministic rules under measured record noise, with pre-registered hypotheses and reported refutations alongside positive results. The repository's committed experiments run at smoke scale; its $10^8$–$10^9$ claims are explicitly treated as extrapolations rather than live production results.

## Named Entity Recognition in clinical notes at billion documents scale
...
![Clinical Notes NER](clinical_ner_pipeline.svg)
...
> **In progress — Hybrid de-identification: a guideline-driven LLM paired with a fine-tuned token classifier** — [note-deid](https://github.com/sinaghadermarzi/note-deid)
>
...
![Single Cell with ML / LLM](singlecell_ml.svg)
...
![Protein Sequence ML](protein_sequence_ml.svg)
